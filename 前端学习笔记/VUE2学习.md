...
Axios 请求成功后返回的对象结构大致如下：

| 字段名         | 含义说明                                         |
| -------------- | ------------------------------------------------ |
| **data**       | 响应体数据（后端返回的业务数据，最常用 ☆）       |
| **status**     | HTTP 状态码（200, 404 等 ☆）                     |
| **statusText** | 状态码对应的文本说明（如 OK, Not Found）         |
| **headers**    | 响应头（包含 content-type、token 等）            |
| **config**     | 请求时的配置对象                                 |
| **request**    | 对应的请求对象（浏览器里是 XMLHttpRequest 实例） |
...
| 状态码  | 含义                                        |
| ------- | ------------------------------------------- |
| **200** | 请求成功，服务器返回了正确的数据            |
| **400** | 客户端请求有语法错误，或参数不正确          |
| **401** | 身份验证失败（未登录 / Token 无效）         |
| **403** | 没有权限访问该资源                          |
| **404** | 请求的资源不存在                            |
| **405** | 请求方式不被支持（如用 POST 调用 GET 接口） |
| **500** | 服务器内部错误                              |

------

📌 **实战技巧**：
 在项目里通常会对 `status` 做统一处理，例如：
